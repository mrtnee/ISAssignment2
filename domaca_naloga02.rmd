...
We will only use the 100 most frequent terms in our term document matrix, because there is almost no difference in model accuracy if we select more terms. Higher number of features also means more training time.
```{r}
...
most_frequent = most_frequent[1:100]
...
As we can see from the table above, all algorithms have almost the same accuracy, but not much better than majority classifier (which has around 0.73 accuracy on both train and test data).
